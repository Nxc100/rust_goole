...
4. 想更快可勾选 **「⚡ 高效模式」** 并拖动「并行浏览器数」（多开浏览器同时翻译）；
5. 点「▶ 开始翻译」。期间会弹出 Chrome 窗口自动操作，**请勿关闭它们**；
6. 右侧实时显示每张图的状态（✔成功 / ×失败）、进度条（高效模式显示「并行 N 个进行中」）、日志；翻译完点「📂 打开输出目录」查看。

### ⚡ 高效模式（多浏览器并行）

勾选「高效模式」后会**同时开多个浏览器并行翻译**，速度接近成倍提升（例：9 张图，单浏览器约 60 秒，
3 浏览器并行约 38 秒）。要点：

- 并行数越高越快，但越占 CPU/内存；过高还可能被 Google 限流，建议 **2–4**。
- 异步收集结果，**完成一张立即保存一张**；任一张失败会**自动重试 1 次**以保证质量。
- 子目录结构、原名保存等行为与普通模式完全一致。
...
# ⚡ 高效模式：3 个浏览器并行
./run.ps1 -i "D:\图片\input" -o "D:\图片\out_en" -t en -j 3

# 或直接调用可执行文件
target\release\google_img_translate.exe -i "D:\图片\input" -o "D:\图片\out_en" -t en -j 3
```
...
| `-j, --concurrency <N>` | 高效模式：并行浏览器数（多开浏览器同时翻译，1=串行） | `1` |
...
- `GIMG_CONCURRENCY=<N>`：N≥2 时自动开启高效模式并设并行浏览器数
